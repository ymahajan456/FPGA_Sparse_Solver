...
inline bool merrgeSelections(int i, int j, vector<vector<int>> & selectionPriorityTable,
                    ...
                    vector<vector<int>> & selectionWritePortTable, 
                    ...
                    int readyCount, int BRAMCount)
{
    ...
    for(k = 0; k < BRAMCount; k++)
    {
        ...
        if((selectionWritePortTable[i][k] + selectionWritePortTable[j][k]) > BRAMPortsCount[k])
            return(false);
    }
    ...
    for(k = 0; k < BRAMCount; k++)
    {
        ...
        // write port
        selectionWritePortTable[j][k] += selectionWritePortTable[i][k];
    }
    ...
    for(auto node : mayBeReadyNodes)
    {
        if(isMACType(node))
        {
            ...
            for(auto j : node->readyMULPairIndex)
            {
                ...
                // have to check if can be read or live
            ...
            if(node->schedulableMULPairIndex.size() > 0)
            {
...
void scheduleNoReg(Graph & lGraph, HardwareConfig & hwConfig, Schedule & schedule)
{
    ...
    // required write ports for selection
    vector<vector<int>> selectionWritePortTable(SELECTION_TABLE_SIZE, vector<int>(MAX_BRAMS, 0));
    ...
    while(numScheduledNodes < numNodesInGraph)
    {
        ...
        std::fill(selectionWritePortTable.begin(), selectionWritePortTable.end(), vector<int>(MAX_BRAMS, 0));
        ...
        for(auto node : readyNodes)
        {
            if(isMACType(node))
            {
                ...
                for(int pair : node->schedulableMULPairIndex)
                {
                    ...
                    // fill write data
                    selectionWritePortTable[i][node->matPointer->BRAMInd[node->memMapIndex]] += 1;

                    // fill selectionNodeTable
                    selectionNodeTable[i] = node;

                    // fill selectionPairIndexTable
                    selectionPairIndexTable[i] = pair;
        ...
        for(i = 0; i < activeSelectionTableSize -1 ; i++)
        {
            ...
            {
                #ifdef PRINT_DEBUG_0
                if(clockCycle == 33 && i == 0 && j == 0)
                {
                    cout << "add break point" << endl;
                }
                #endif
                
                // operations from same nodes can't be merged
                if(selectionNodeTable[i]->name == selectionNodeTable[j]->name)
                {
                    continue;
                }
                ...
                selectionWritePortTable, freeBRAMPortsCount, BRAMPortsCount, 
                activeSelectionTableSize, BRAMPortsCount.size());
        ...
        updateDirtyMemoryStatus(previousAssignedNodes, completedWrites);
        ...
        cout << "Clock Cycle: " << clockCycle << " Done Nodes: " << numScheduledNodes << " / " << numNodesInGraph << endl;
